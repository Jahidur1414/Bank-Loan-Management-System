...
#=============================Functions================================
    ...
    def update(self):
        if self.loalId.get()=='':
            ...
            cur.execute("update employee set name=%s,years=%s,rate=%s,Mpayment=%s,Tpayment=%s,mobile= %s ,AadharNumber=%s,Address=%s,Pincode=%s,Amount=%s where empId=%s",(self.name.get(),
                                                                                                                                                                         self.year.get(),
                                                                                                                                                                        self.rate.get(),
                                                                                                                                                                        self.mpay.get(),
                                                                                                                                                                        self.tpay.get(),
    ...
    def delete(self):
        if self.loalId.get()=='':
            ...
            con = pymysql.connect(host="localhost", user="root", password="", database="test")
            cur = con.cursor()
            cur.execute("delete from employee where empId=%s",self.loalId.get())
            con.commit()
            con.close()
            self.fatch_data()
            self.reset()
